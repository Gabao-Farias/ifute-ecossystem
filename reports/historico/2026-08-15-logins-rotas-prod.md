...
**Gerado em:** 2026-08-15 11:34 BRT
**Janela analisada:** `--since 4 days ago` → 2026-08-11 11:35 a 2026-08-15 11:34 BRT
...
| Métrica | Valor |
|---|---|
| Requisições analisadas | 24255 |
| Linhas JSON lidas / ignoradas | 87716 / 330 |
| Logins observados | 6 |
| Logins atribuídos a um usuário | 6 de 6 |
| Usuários distintos autenticados | 3 |
| IPs distintos | 8 |
| Requisições autenticadas | 23406 (96.5%) |
| — destas, de automação (jobber/cron) | 23040 (95.0%) |
| — destas, de usuários reais | 366 (1.5%) |
| Requisições abortadas pelo cliente | 0 |

**Por app e por classe de status:**
| Dimensão | Valor | Requisições |
|---|---|---|
| app | webhook | 23044 (95.0%) |
| app | director | 564 (2.3%) |
| app | images | 429 (1.8%) |
| app | backoffice | 138 (0.6%) |
| app | mobile | 72 (0.3%) |
| app | outros | 8 (0.0%) |
| status | 2xx | 23481 (96.8%) |
| status | 3xx | 735 (3.0%) |
| status | 4xx | 39 (0.2%) |
...
| Campo | Valor |
|---|---|
| Primeiro acesso no período | 2026-08-11 12:43 BRT |
| Último acesso no período | 2026-08-15 11:33 BRT |
| Requisições autenticadas | 271 |
| Sessões | 20 (corte de inatividade: 30 min) |
| Logins no período | 4 |
| IPs | `189.7.228.9` (271) |
| Plataforma | navegador (Chrome) (271) |
| Respostas ≥ 400 | 0 |

**Rotas mais acessadas:**
| Rota | Requisições | Erros | p50 | p95 |
|---|---|---|---|---|
| `GET /director/private/recents/admins` | 74 | — | 16 ms | 25 ms |
| `GET /director/private/recents/places` | 74 | — | 18 ms | 28 ms |
| `GET /director/private/recents/users` | 74 | — | 16 ms | 27 ms |
| `GET /director/private/reports/places/ranking` | 11 | — | 8 ms | 11 ms |
| `GET /director/private/reports/revenue/monthly` | 11 | — | 8 ms | 25 ms |
| `GET /director/private/config/platform-pix-key` | 9 | — | 5 ms | 15 ms |
| `GET /director/private/withdrawal/history` | 9 | — | 6 ms | 19 ms |
| `GET /director/private/withdrawal/balance` | 8 | — | 11 ms | 28 ms |
...
| 2026-08-15 11:33 | 0s | 3 | `/director/private/recents/users` | `/director/private/recents/places` |
...
|---|---|---|---|---|
| ifute-jobber (crons internos) | 23040 | `POST /webhook/internal/stripe` (23040) | 2026-08-11 11:35 | 2026-08-15 11:34 |
...
|---|---|---|---|---|---|
| `POST /webhook/internal/stripe` | webhook | 23040 | — | 21 ms | 41 ms |
| `GET /images/public/:id.png` | images | 238 | — | 1 ms | 4 ms |
| `GET /images/public/:id.jpg` | images | 191 | — | 1 ms | 3 ms |
| `GET /director/private/recents/admins` | director | 76 | 2 (2.6%) | 15 ms | 25 ms |
| `GET /director/private/recents/places` | director | 76 | 2 (2.6%) | 18 ms | 28 ms |
| `GET /director/private/recents/users` | director | 76 | 2 (2.6%) | 16 ms | 27 ms |
| `OPTIONS /director/private/recents/admins` | director | 75 | — | 1 ms | 2 ms |
| `OPTIONS /director/private/recents/places` | director | 75 | — | 1 ms | 3 ms |
| `OPTIONS /director/private/recents/users` | director | 75 | — | 1 ms | 2 ms |
| `GET /director/private/reports/places/ranking` | director | 11 | — | 8 ms | 11 ms |
| `GET /director/private/reports/revenue/monthly` | director | 11 | — | 8 ms | 25 ms |
...
| `GET /director/private/config/platform-pix-key` | director | 10 | 1 (10.0%) | 5 ms | 15 ms |
...
| `GET /backoffice/private/auth/login/check` | 3 |
...
| `GET /backoffice/private/adminUser/finances/summary` | 1 |
| `GET /backoffice/private/withdrawal/balance` | 1 |
...
- **Rotas públicas não têm usuário.** `3.5%` das requisições são anônimas (públicas, webhooks, imagens, scanners); elas contam nos rankings globais mas não na seção por usuário.
